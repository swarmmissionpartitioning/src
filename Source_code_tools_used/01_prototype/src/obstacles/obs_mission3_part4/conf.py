...
INHERITED = True #when it is true, coordinates are set manually in planner.py

XY_START = np.array([1.78383321, 0.59509524+2.0]) #for p1
XY_GOAL = np.array([-3.6, -3.8]) #for p1

APPLIED_MODIFIED_P = True
MODIFIED_P = np.genfromtxt("input/modified_p/m3_p4/modified_p.csv", delimiter = ',')
# the latter should be start point

'''
Note that `p.csv` should be like below:
-2,-4  <== goal point (supposed to be XY_GOAL)
-2.2,2.2
-3.5,4 <== the first waypoint (start point will be added)
'''
