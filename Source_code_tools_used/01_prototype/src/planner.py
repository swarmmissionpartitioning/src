...
def fuzz_one(args, obstacle_type, feedback_type,  check_crash=False, manual_mode=False, param_setting='None', randometest_order=1):
    ...
    if params.mode_replay is True:
        ...
        print("[dbg]traj_global: "+str(traj_global))
    ...
    print("[traj_global[0, :]]: " + str([traj_global[0, :]]))
    ...
    if params.mode_replay is True:
        ...
        if params.mode_randomtesting is True:
            ...
            print("[dbg]rbt.sp_global:"+str(rbt.sp_global))
            ...
            print("[dbg1]rbt.sp_global:"+str(rbt.sp_global))
    ...
    for x in tqdm(range(MAX_TICK)):
        ...
        # print("[dbg]simul_tick: "+str(simul_tick))
        if SPECIAL_MOVE_OBS_IN_SIM and simul_tick == 100:
            OBSTACLES[3] = move_special_obs_in_sim(OBSTACLES[3])
            print("obs[3] is switched")
...
simul_tick [%d] in zone_[%d] at %f %f %f %f" % \
        ...
        # print("[temp_log] sp = "+str(rbt.sp))
        # print("[temp_log] spglobal = "+str(rbt.sp_global))
        ...
        if RECORD_COORD:
            msg_coord = "p_seed %d leader %f %f f1 %f %f f2 %f %f f3 %f %f" % \
                (p_seed, rbt.sp[0], rbt.sp[1], robots[1].sp[0], robots[1].sp[1],
        ...
        if condition_1:
            ...
            if special_condition_1:
                print("[log] go anyway")
        # print("[dbg]traj_global.shape[0]: "+str(traj_global.shape[0]))
...
simul_tick [%d] with [%f] in zone_[%d] speedX [%f] obsSize [%f]" % \
    ...
    if REPORT:
        reports.ep_leader = rbt.sp
        reports.ep_f1 = robots[1].sp
        reports.ep_f2 = robots[2].sp
        reports.ep_f3 = robots[3].sp

        msg = f"final pose: {reports.ep_leader}, {reports.ep_f1}, {reports.ep_f2}, {reports.ep_f3}"
        writeFile(FINAL_REPORT, msg)
...
def AdSwarm(feedback_type, rand_start, rand_end):
    ...
    if REPORT:
        msg = f"first pose: {des1}, {des2}, {des3}, {des4}"
        writeFile(FINAL_REPORT, msg)
        msg = f"goal: {XY_GOAL}"
        writeFile(FINAL_REPORT, msg)
