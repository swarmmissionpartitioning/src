...
def rrt_path(obstacles, xy_start, xy_goal, params):
    print("rrt_path's xy_start: ["+str(xy_start)+"] xy_goal: "+str(xy_goal))
    ...
    while not nearGoal: # and iters < maxiters:
        ...
        if rnd < params.goal_prob:
            ...
            xy = np.array([float(np.random.rand(1))*2*params.world_bounds_x[1]-params.world_bounds_x[1], float(np.random.rand(1))*2*params.world_bounds_y[1]-params.world_bounds_y[1]]) # Should be a 2 x 1 vector
